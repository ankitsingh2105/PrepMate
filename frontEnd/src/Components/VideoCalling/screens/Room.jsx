...
export default function Room({ windowWidth, roomWidth = 640, roomHeight = 360, direction = 'row' }) {
  ...
  const initializeStream = useCallback(async () => {
    try {
      ...
      console.log('Initialized stream:', {
        audioTracks: audioTracks.map(t => ({ enabled: t.enabled, label: t.label, readyState: t.readyState })),
        videoTracks: videoTracks.map(t => ({ enabled: t.enabled, label: t.label, readyState: t.readyState })),
      ...
      toast.error(`Camera/Mic access failed: ${error.message}. Check permissions.`, { autoClose: 3000 });
      return null;
    }
  }, []);
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (userDetails && userDetails.email) {
      ...
      toast.error('User not authenticated', { autoClose: 1500 });
    }
  ...
  useEffect(() => {
    if (myStream) {
      console.log('My stream tracks:', myStream.getTracks().map(t => ({ kind: t.kind, enabled: t.enabled, readyState: t.readyState })));
    }
  }, [myStream]);
  ...
  const sendStreams = useCallback(() => {
    ...
    console.log('Adding tracks:', tracks.map(t => ({ kind: t.kind, enabled: t.enabled, label: t.label, readyState: t.readyState })));
  ...
  const handleNegoNeeded = useCallback(async () => {
    try {
      const offer = await caller.getOffer();
    ...
    peer.webRTCPeer.addEventListener('track', async (e) => {
      ...
      if (remoteStreams && remoteStreams[0]) {
        ...
        console.log('Remote tracks:', {
          ...
          streamActive: remoteStreams[0].active,
        });
        ...
        <div className="flex flex-col items-center">
          <div className="bg-purple-50 text-gray-700 py-2 px-4 rounded-lg shadow-sm mb-4 flex items-center justify-center space-x-2 w-auto max-w-md mx-auto">
            ...
            <div className="flex items-center bg-white px-3 py-1 rounded border border-purple-200">
              <span className="text-xs text-gray-600 truncate max-w-[200px]">{currentPageUrl}</span>
            </div>
            <button
                  ...
                  ref={(video) => {
                    if (video && myStream) {
                      ...
                      video.onloadedmetadata = () => {
                        ...
                        console.log('Local video loaded:', myStream.getVideoTracks()[0].getSettings());
                      };
                    }
                  }}
                    ...
                    ref={(video) => {
                      if (video && remoteStream) {
                        ...
                        video.onloadedmetadata = () => {
                          ...
                          console.log('Remote video loaded:', remoteStream.getVideoTracks()[0].getSettings());
                        };
                      }
                    }}
