...
export default function UserProfile() {
  ...
  const handleBookingCancel = async (
    ...
    try {
      await axios.post(
        `${backEndLink}/user/cancelBooking`,
        {
          myUserId,
          otherUserId,
          myTicketId,
      ...
      setBookings(
        bookings.filter((booking) => booking.bookingTime !== bookingTime)
      );
      setRefresh((prev) => !prev);
      socket.emit("notification", {
        message: `Booking for ${bookingTime} is cancelled by the user`,
        otherUserId,
      });
      dispatch(handleUserInfo());
    } catch (error) {
    ...
    <>
      {isLoggedIn === "false" ? (
        <>
          <center>
            <br />
            <b>Please login to see details</b>
          </center>
        </>
      ) : (
        <>
                      ...
                      {bookings.map((booking, index) => (
                        ...
                        >
                          ...
                          <span className="flex text-center">
                            <span
                              onClick={() =>
                                handleRoomIDnavigate(
                                  booking.roomID,
                                  booking.mockType
                                )
                              }
                              className="h-7 w-7 rounded-full bg-purple-100 text-purple-800 flex items-center justify-center font-bold text-sm hover:cursor-pointer"
                            >
                              Link
                            </span>
                          </span>
                    ...
                    <div className="bg-gray-50 rounded-xl p-8 text-center border border-dashed border-gray-300">
                      <div className="flex flex-col items-center justify-center space-y-3">
                        ...
                        <p className="text-gray-500 text-sm max-w-xs">
                          You don't have any upcoming mock interviews scheduled.
                          Book one to prepare for your next opportunity.
                        </p>
                      </div>
                    </div>
                  )}
